...
def train_real(epochs=30, batch_size=256, data="go1_data.npz",
    ...
    if use_plan is not None:
        cfg.use_plan = use_plan            # ablation: no latent plan
    device = pick_device()
    print(f"device: {device} | tag={tag} | dyn={cfg.weights.dyn} | use_plan={cfg.get('use_plan', True)}")
    ...
    for epoch in range(epochs):
        ...
        if running["total"] < best:
            best = running["total"]
            torch.save(model.state_dict(), f"checkpoint_{tag}.pt")
...
if __name__=="__main__":
    ...
    if arg=="train":
        ...
        train_real(tag="noplan", use_plan=False)        # ablation: no latent plan
    else:
        overfit()
